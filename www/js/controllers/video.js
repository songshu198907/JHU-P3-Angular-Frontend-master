      ...
      $scope.getPatientSurvey = function () {
        ...
        $userSurvey.get_patients_survey({}, function (collection) { // Success callback
          ...
          if (!collection || collection.length == 0) {
            ...
            var lNumOfMaternals = _.filter(lVideos.maternals, function (video) {
              return video.is_complete == 0;
            });
            var lNumbOfPediatrics = _.filter(lVideos.pediatrics, function (video) {
              return video.is_complete == 0;
            });
            var postIncompleteMaternals = _.filter(postVideos.maternals, function (video) {
              return video.is_complete == 0;
            });
            var postIncompletePediatrics = _.filter(postVideos.pediatrics, function (video) {
              return video.is_complete == 0;
            })
            $scope.incomplete = lNumOfMaternals.length + lNumbOfPediatrics.length + postIncompleteMaternals.length + postIncompletePediatrics.length;
            console.debug("There are ", $scope.incomplete, " video surveys not complete, including lNumOfMaternals: ",
              lNumOfMaternals, ", lNumbOfPediatrics:", lNumbOfPediatrics, ", postIncompleteMaternals :", postIncompleteMaternals, "，postIncompletePediatrics：", postIncompletePediatrics);
            if ($scope.incomplete == 0) {
              ...
              if (lVideos.maternals.length > 0 || lVideos.pediatrics.length > 0 || postIncompleteMaternals.length > 0 || postIncompletePediatrics.length > 0) {
                if (lVideos.maternals.length > 0) {
                  var lVideo = _.filter(lVideos.maternals, function (video) {
                    return video.is_complete == 0;
                  });
                  lVideo = _.sortBy(lVideo, function (video) {
                  ...
                  if (lVideo.length > 0) {
                    ...
                    var lVideo = _.filter(lVideos.pediatrics, function (video) {
                      return video.is_complete == 0;
                    });
                    lVideo = _.sortBy(lVideo, function (video) {
                      return video.sort_order;
                ...
                } else if (postIncompleteMaternals.length > 0) {
                  postIncompleteMaternals = _.sortBy(postIncompleteMaternals, function (video) {
                    return video.sort_order;
                  })
                  $scope.video = postIncompleteMaternals[0];
                  if ($scope.video.video_position != null) {
                    $scope.startTime = $scope.video.video_position;
                  }
                } else {
                  postIncompletePediatrics = _.sortBy(postIncompletePediatrics, function (video) {
                    return video.sort_order;
                  })
                  $scope.video = postIncompletePediatrics[0];
                  if ($scope.video.video_position != null) {
                    $scope.startTime = $scope.video.video_position;
                  }
                }
                    ...
                    setTimeout(function(){
                      ...
                      if ($scope.after30Day) {
                        $userSurvey.get_post_video_survey({}, function (collection) {
                          ...
                          console.log("30-day post video not found");
                        })
                      }
