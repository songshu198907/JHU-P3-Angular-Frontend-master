  ...
  .controller('LoginCtrl', ['$rootScope', '$scope', '$mdDialog', 'AuthCtrl', '$state', '$user', '$userSurvey', 'cfpLoadingBar', '$sessionStorage', '$ionicViewSwitcher', '$setting', '$authenticationStorageService', '$ionicLoading', '$setting',
    function ($rootScope, $scope, $mdDialog, AuthCtrl, $state, $user, $userSurvey, cfpLoadingBar, $sessionStorage, $ionicViewSwitcher, $setting, $authenticationStorageService, $ionicLoading, $setting) {
      ...
      $scope.getUserSettings = function () {
        $setting.all({}, function (collection) { // Success callback
            ...
            if (collection.length == 0) {
              ...
              if (collection[0].hasOwnProperty('matched_videos')) {
                ...
                if (lVideos.maternals.length > 0 || lVideos.pediatrics.length > 0) {
                  ...
                  var incompleteLength = _.filter(lVideos.maternals, function (video) {
                    return video.is_complete == 0;
                  }).length + _.filter(lVideos.pediatrics, function (video) {
                    return video.is_complete == 0;
                  }).length;
              ...
              } else if (collection.hasOwnProperty('30day_matched_videos')) {
                var postVideos = collection[0]['30day_matched_videos'];
                if (postVideos.maternals.length > 0 || postVideos.pediatrics.length > 0) {
                  var incompleteSurveys = _.filter(postVideos.maternals, function (video) {
                    return video.is_complete == 0;
                  }).length + _.filter(postVideos.pediatrics, function (vedio) {
                    return vedio.is_complete == 0;
                  }).length;
                  if (incompleteSurveys > 1) {
                    $state.go("home.videos");
                  } else if (incompleteSurveys > 0) {
                    $state.go("videoTiles");
                  } else if (collection[0].is_complete == false) {
                    $scope.surveyStart();
                  }
